...
Из таблицы видно, что программа, как ни странно, замедлилась и относительное ускорение меньше 1, поэтому нет особого смысла осуществлять профилирование. Функция `strcmp-avx2` оказалась быстрее, так как она основана на `SIMD` инструкциях, о которых можно прочитать на репозитории [SIMD](https://github.com/Art8mish/SIMD). Откатим эту оптимизацию отбросим и попробуем написать свою версию `avx_strcmp`.
...
> callgrind_annotate callgrind.out.<NUM>
