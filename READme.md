...
Все результаты профилирований можно посмотреть самому, в папке `.callgrind` хранятся все исходные файлы с данными. Для того что бы просмотреть информацию нужно прописать в консоль `callgrind_annotate callgrind.out.NUM`, где NUM - индивидуальный номер файла.
